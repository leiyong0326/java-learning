...
public class StringUtil {
	...
	public static String getNonceStr(int count) {
		return getHexString((int) (NumberUtil.random((int) Math.pow(16, count))),count);
	}
	/**
	 * 获取十六进制数据
	 * @param number 被转化的数据
	...
	public static String fullStringBefore(String src,char full,int length){
		if (length>0) {
			if (src.length()>length) {
				...
				return getNString(full, length-src.length())+src;
			}
		}
		return src;
	}
	 ...
	 * @return 返回{tar前的字符串,tar后的字符串},如str=offott,tar=o,startIndex=1,{"ff","tt"},若未找到则返回{"",str}
	 */
